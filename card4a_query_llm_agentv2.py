    ...
    return ChatPromptTemplate.from_messages(
        [
            ('system"'
             ...
             'Use the tools "transactions_search_tool", "consumer_daily_transactions_search_tool", "consumer_monthly_transactions_search_tool", "consumer_yearly_transactions_search_tool", "portfolio_daily_transactions_search_tool", "portfolio_monthly_transactions_search_tool", "portfolio_yearly_transactions_search_tool" wisely to search in the context of transactions. '
             'If the retrieved data is not related or isn\'t sufficient to answer exactly the question, just say "Eu não tenho dados suficientes para responder essa instrução." or shortly explain why. '
             'Do not try to guess and never make up data, this is very important. '
             'Question: {input}'),
...
def build_rag_chain():
    ...
    tools = [
        create_retriever_tool(
            ...
            'Document example: <doc>O cliente Lucas Viana (CPF: 92106574894) efetuou a transação de ID "0" com cartão de crédito PLATINUM de R$ 1536.10 no dia 2 do mês de janeiro do ano de 2023 (02/01/2023) para o estabelecimento "Caldeira"</doc>'
        ),
        create_retriever_tool(
            ...
            'Document example: <doc>Resumo diário das transações com cartão de crédito do cliente Lucas Viana (CPF: 92106574894) no dia 2 do mês de janeiro do ano de 2023 (02/01/2023), com uma contagem total de 2 transações e valor total de R$ 6023.44, o valor médio é de R$ 3011.72, o valor da maior transação é de R$ 4487.33 e o valor da menor transação é de R$ 1536.10. Dentre elas foram realizadas 1 com cartão PLATINUM, 1 com cartão INTERNACIONAL.</doc>'
        ),
        create_retriever_tool(
            ...
            'Document example: <doc>Resumo mensal das transações com cartão de crédito do cliente Lucas Viana (CPF: 92106574894) no mês de janeiro do ano de 2023, com uma contagem...</doc>',
        ),
        create_retriever_tool(
            get_retriever(vs, 1, {'data_granularity': 'transaction_year/portfolio_id/consumer_id'}),
            'consumer_yearly_transactions_search_tool',
            ...
            'Document example: <doc>Sumário anual de transações com cartão de crédito do cliente Lucas Viana (CPF: 92106574894) no ano de 2023, com uma contagem...</doc>',
        ),
        create_retriever_tool(
            ...
            'Document example: <doc>Sumário diário de transações com cartão de crédito de todos os clientes da carteira "0" no dia 2 do mês de janeiro do ano de 2023 (02/01/2023), com uma contagem...</doc>'
        ),
        create_retriever_tool(
            ...
            'Document example: <doc>Sumário mensal de transações com cartão de crédito de todos os clientes da carteira "0" para o mês de janeiro do ano de 2023 (01/2023), com uma contagem...<doc>'
        ),
        create_retriever_tool(
            get_retriever(vs, 1, {'data_granularity': 'transaction_year/portfolio_id'}),
            'portfolio_yearly_transactions_search_tool',
            ...
            'Document example: <doc>Sumário anual de transações com cartão de crédito de todos os clientes da carteira "0" para o ano de 2023, com uma contagem...</doc>'
        ),
    ]
